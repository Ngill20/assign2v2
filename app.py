...
server = 'part2server.database.windows.net'
...
def upload():
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.csv'):
            ...
            # Read the cleaned CSV file while skipping blank lines
            ...
            for index, row in df_cleaned.iterrows():
                try: 
                    time        = safe_datetime(row['time'])
                    latitude    = safe_float(row['latitude'])
                    longitude   = safe_float(row['longitude'])
                    depth       = safe_float(row['depth'])
                    mag         = safe_float(row['mag'])
                    magType     = str(row['magType']) if pd.notna(row['magType']) else None
                    nst         = safe_int(row['nst'])
                    gap         = safe_float(row['gap'])
                    dmin        = safe_float(row['dmin'])
                    rms         = safe_float(row['rms'])
                    net         = str(row['net']) if pd.notna(row['net']) else None
                    id_         = str(row['id'])
                    updated     = safe_datetime(row['updated'])
                    place       = str(row['place']) if pd.notna(row['place']) else None
                    type_       = str(row['type']) if pd.notna(row['type']) else None
                    h_error     = safe_float(row['horizontalError'])
                    d_error     = safe_float(row['depthError'])
                    m_error     = safe_float(row['magError'])
                    magNst      = safe_int(row['magNst'])
                    status      = str(row['status']) if pd.notna(row['status']) else None
                    loc_src     = str(row['locationSource']) if pd.notna(row['locationSource']) else None
                    mag_src     = str(row['magSource']) if pd.notna(row['magSource']) else None
                    print(f"Row {index}: rms={rms} ({type(rms)}), net={net} ({type(net)}), magNst={magNst} ({type(magNst)})")
                    cursor.execute("""
                        INSERT INTO Earthquakes (
                    ...
                    """, time, latitude, longitude, depth, mag, magType, nst, gap, dmin, rms,
                        net, id_, updated, place, type_, h_error, d_error, m_error, magNst,
                        status, loc_src, mag_src)
                except Exception as e:
